...
- **Runtime: Node 24.18.0 from the `heirloom` conda env** (`conda create -n heirloom -c conda-forge nodejs=24`). System Node is 18.19.1, which the current toolchain rejects. Every command must put the env first on PATH — `export PATH=~/miniconda3/envs/heirloom/bin:$PATH` — because `npm` resolves `node` from PATH rather than from its own location, so calling npm by absolute path silently runs it under system Node 18.
- **Toolchain versions are registry-verified, not recalled** (checked 2026-07-29): typescript 7.0.2, vite 8.1.5, vitest 4.1.10, playwright 1.62.0. The versions originally written into this plan (vite 5 / vitest 2 / typescript 5.6.0) came from recall; `typescript@5.6.0` does not exist in the registry at all. TypeScript 7 is the native-compiler rewrite, so Task 6 Step 5 (`tsc --noEmit`) is the gate that proves it works on this codebase; if it misbehaves, fall back to the latest 6.x and record the reason.
- Curves are **emergent from the growth path** (spec §3). No task fits cubic bezier control points to anything.
- Two deviations from spec §3 are deliberate. (a) §3 assigns stroke drawing to `render/strokes.ts` but never says where _petal_ drawing lives; this plan adds `render/petals.ts` rather than overloading `strokes.ts`. (b) §3 lists `growth/agent.ts` for the tip, but a tip is a pure type with no behaviour, so `Tip` lives in `src/types.ts` with the other contracts and no `agent.ts` is created.
...
Expected: FAIL — cannot resolve `../src/rng`. If it fails for any _other_ reason, stop and read the error; a test that fails for the wrong reason proves nothing.
...
describe("growPlant", () => {
  ...
  it("grows upward when droop is zero and downward-biased when droop is high", () => {
    ...
    const lowest = (segs: typeof upright.segments) =>
...
Canvas drawing is verified by the critic gate in Task 7, not by unit tests. What _is_ unit-testable is the animation filter, so that is what Step 1 tests.
...
const page = await browser.newPage({
  viewportSize: { width: 960, height: 1120 },
});
...
await page.waitForFunction(() => window.__lookdevReady === true, {
  timeout: 15_000,
});
